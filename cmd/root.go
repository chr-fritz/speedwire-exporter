...
func NewRootCommand() (*cobra.Command, *RootOptions) {
	...
	cmd := &cobra.Command{
		Use:   "speedwire-exporter",
		Short: "Exports SMA Speedwire values to Prometheus",
	}

	cmd.PersistentFlags().StringVar(&rootOptions.configFile, "config", "", "config file (default is $HOME/.speedwire-exporter.yaml)")
...
func (o *RootOptions) initConfig() {
	if o.configFile != "" {
		...
		// Search config in home directory with name ".speedwire-exporter" (without extension).
